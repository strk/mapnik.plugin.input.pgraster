...
pgraster_featureset::pgraster_featureset(GDALDataset& dataset,
                                 int band,
                                 pgraster_query q,
...
{
    MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Closing Dataset=" << &dataset_;
...
feature_ptr pgraster_featureset::next()
{
    ...
    {
        first_ = false;
        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Next feature in Dataset=" << &dataset_;
        return boost::apply_visitor(query_dispatch(*this), gquery_);
    }
    return feature_ptr();
}

feature_ptr pgraster_featureset::get_feature(mapnik::query const& q)
{
    ...
    /*
      ...
      MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: dx_=" << dx_ << " dx=" << dx << " dy_=" << dy_ << "dy=" << dy;
#endif
    */
    ...
    MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Raster extent=" << raster_extent_;
    MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: View extent=" << intersect;
    MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Query resolution=" << boost::get<0>(q.resolution()) << "," << boost::get<1>(q.resolution());
    MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: StartX=" << x_off << " StartY=" << y_off << " Width=" << width << " Height=" << height;
    ...
    if (width > 0 && height > 0)
    {
        ...
        if (im_width > 0 && im_height > 0)
        {
            ...
            MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Image Size=(" << im_width << "," << im_height << ")";
            MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Reading band=" << band_;
            ...
            else // working with all bands
            {
                for (int i = 0; i < nbands_; ++i)
                {
                    ...
                    case GCI_RedBand:
                        red = band;
                        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Found red band";
                        break;
                    case GCI_GreenBand:
                        green = band;
                        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Found green band";
                        break;
                    case GCI_BlueBand:
                        blue = band;
                        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Found blue band";
                        break;
                    case GCI_AlphaBand:
                        alpha = band;
                        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Found alpha band";
                        break;
                    case GCI_GrayIndex:
                        grey = band;
                        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Found gray band";
                        break;
                    case GCI_PaletteIndex:
                    {
                        grey = band;
#ifdef MAPNIK_LOG
                        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Found gray band, and colortable...";
                        GDALColorTable *color_table = band->GetColorTable();

                        if (color_table)
                        {
                            int count = color_table->GetColorEntryCount();
                            MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Color Table count=" << count;

                            for (int j = 0; j < count; j++)
                            {
                                const GDALColorEntry *ce = color_table->GetColorEntry (j);
                                if (! ce) continue;
                                MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Color entry RGB=" << ce->c1 << "," <<ce->c2 << "," << ce->c3;
                            }
                        }
#endif
                        break;
                    ...
                    case GCI_Undefined:
                        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Found undefined band (assumming gray band)";
                        grey = band;
                        break;
                    default:
                        MAPNIK_LOG_WARN(gdal) << "pgraster_featureset: Band type unknown!";
                        break;
                    }
                }
                if (red && green && blue)
                {
                    MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Processing rgb bands...";
                ...
                {
                    MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Processing gray band...";
                    ...
                    {
                        double apply_nodata = nodata_value_ ? *nodata_value_ : raster_nodata;
                        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: applying nodata value for layer=" << apply_nodata;
                    ...
                    if (color_table)
                    {
                        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: Loading color table...";
                        for (unsigned y = 0; y < image.height(); ++y)
                        {
                            ...
                            for (unsigned x = 0; x < image.width(); ++x)
                            {
                                ...
                                else
                                {
                                    ...
                                    // note - pgraster_translate makes black
                                    row[x] = 0;
                                }
                            }
                        }
                ...
                if (alpha)
                {
                    MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: processing alpha band...";
...
feature_ptr pgraster_featureset::get_feature_at_point(mapnik::coord2d const& pt)
{
    if (band_ > 0)
    {
        ...
        {
            MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: pt.x=" << pt.x << " pt.y=" << pt.y;
            MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: x=" << x << " y=" << y;
...
#ifdef MAPNIK_LOG
void pgraster_featureset::get_overview_meta(GDALRasterBand* band)
{
    int band_overviews = band->GetOverviewCount();
    if (band_overviews > 0)
    {
        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: " << band_overviews << " overviews found!";

        for (int b = 0; b < band_overviews; b++)
        {
            GDALRasterBand * overview = band->GetOverview(b);
            MAPNIK_LOG_DEBUG(gdal) << boost::format("pgraster_featureset: Overview=%d Width=%d Height=%d")
                % b % overview->GetXSize() % overview->GetYSize();
        }
    }
    else
    {
        MAPNIK_LOG_DEBUG(gdal) << "pgraster_featureset: No overviews found!";
    }
    ...
    MAPNIK_LOG_DEBUG(gdal) << boost::format("pgraster_featureset: Block=%dx%d Scale=%f Type=%s Color=%s") % bsx % bsy % scale
